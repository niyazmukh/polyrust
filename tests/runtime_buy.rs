...
use minirust::runtime::on_binance_sample;
...
fn book_ticker_with_active_quotes_and_no_exposure_returns_buy_intent() {
    ...
    inventory.set_user_wss_trusted(true);
    let mut signal = seeded_signal();

    let intent = on_binance_sample(
        ticker_sample(),
        &mut signal,
        &state,
        &inventory,
...
#[test]
fn user_wss_trusted_false_blocks_buy() {
    let now = TsUs(1_777_000_030_010_000);
    let state = active_state(now);
    let inventory = Inventory::new();
    // user_wss_trusted defaults to false → BUY blocked
    let mut signal = seeded_signal();
...
#[test]
fn user_wss_trusted_false_trumps_no_exposure() {
    // When user_wss_trusted is false, has_entry_exposure_or_pending returns true
    // even with zero inventory → BUY is suppressed.
    let inventory = Inventory::new();
    // default trust is false
    assert!(inventory.has_entry_exposure_or_pending(&TokenId::new("yes")));
}
