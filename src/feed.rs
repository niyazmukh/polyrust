...
/// - Sends auth frame scoped to current market:
///   `{"auth": {...}, "markets": ["<condition_id>"], "type": "user"}`
/// - `outgoing` receives official subscription update frames:
...
pub async fn user_feed_loop(
    url: &str,
    auth: UserFeedAuth<'_>,
    current_markets: impl Fn() -> Vec<String> + Send + 'static,
    on_event: impl Fn(Bytes) + Send + 'static,
...
mod tests {
    ...
    #[tokio::test]
    async fn user_feed_sends_auth_frame_and_subscription_update() {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
        let url = format!("ws://{addr}");

        // Single-shot server: accept one connection, read auth and update frames.
        let server = tokio::spawn(async move {
            let (stream, _) = listener.accept().await.unwrap();
            let ws = accept_async(stream).await.unwrap();
            let (_write, mut read) = ws.split();
            let auth = read.next().await;
            let update = read.next().await;
            (auth, update)
        });
        ...
        match first_frame {
            Some(Ok(Message::Text(t))) => {
                ...
                assert_eq!(v["markets"][0], "0xcondition");
